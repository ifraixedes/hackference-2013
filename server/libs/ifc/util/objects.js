...
/**
 * // TODO integrate the possibility to get properties into the Array. Scott has an implementation of it
 *
 * @param {Object} obj Object to get the property value specified by path parameter
 * @param {String} path Complete path, in dot notation
		...
		if (Array.isArray(subObj)) {
			for (p = 0; p < fullObj.length; p++) {
				if (subObj[p] !== undefined) {
					if ((null !== fullObj[p]) && ('object' === typeof fullObj[p])) {
...
module.exports.removeEmptyValues = function (obj, keep) {
	...
	for (var p in obj) {
		...
		if (null === obj[p]) {
			if (undefined === keep['null']) {
				delete obj[p];
			}
		} else if (Array.isArray(obj[p])) {
			if ((0 === obj[p].length) && (undefined === keep['array'])) {
				delete obj[p];
			}
...
var flatten = module.exports.flatten = function (obj, options) {
	...
	for (p in obj) {
		if (((0 > maxDeep) || (0 > maxDeep))&& ('object' === typeof obj[p])) {

			if ((obj[p] instanceof Array) && (true !== flattenArrays)) {
					flattened[p] = obj[p];
			} else {
